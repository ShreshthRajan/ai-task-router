...
def sample_developers(db_session):
    ...
    # Create proper 768-dimensional vectors
    skill_vector_1 = [0.9, 0.7, 0.5, 0.8, 0.6] + [0.0] * 763
    skill_vector_2 = [0.3, 0.8, 0.9, 0.4, 0.5] + [0.0] * 763
    skill_vector_3 = [0.8, 0.6, 0.7, 0.9, 0.8] + [0.0] * 763
    
    developers = []
    
    # Senior Python Developer
    dev1 = Developer(
        github_username="senior_python_dev_test",
        name="Alice Johnson",
        email="alice.test@example.com",
        skill_vector=json.dumps(skill_vector_1),
        primary_languages=json.dumps({"python": 0.9, "sql": 0.7, "javascript": 0.5}),
        domain_expertise=json.dumps({"backend": 0.9, "api": 0.8, "database": 0.7}),
    ...
    dev2 = Developer(
        github_username="junior_frontend_dev_test",
        name="Bob Smith",
        email="bob.test@example.com",
        skill_vector=json.dumps(skill_vector_2),
        primary_languages=json.dumps({"javascript": 0.8, "typescript": 0.7, "css": 0.9}),
        domain_expertise=json.dumps({"frontend": 0.9, "ui_ux": 0.8, "react": 0.8}),
    ...
    dev3 = Developer(
        github_username="ml_engineer_test",
        name="Carol Davis",
        email="carol.test@example.com",
        skill_vector=json.dumps(skill_vector_3),
        primary_languages=json.dumps({"python": 0.9, "r": 0.7, "sql": 0.8}),
        domain_expertise=json.dumps({"machine_learning": 0.9, "data_science": 0.8, "backend": 0.7}),
...
def sample_tasks(db_session):
    ...
    task1 = Task(
        ...
        repository="backend-service-test",
        priority="high",
        labels=json.dumps(["backend", "redis", "performance"]),
        ...
        required_skills=json.dumps({"python": 0.8, "redis": 0.9, "backend": 0.8}),
        status="open"
    )
    ...
    task2 = Task(
        ...
        repository="frontend-app-test",
        priority="medium",
        labels=json.dumps(["frontend", "react", "ui"]),
        ...
        required_skills=json.dumps({"javascript": 0.7, "react": 0.8, "typescript": 0.6}),
        status="open"
    )
    ...
    task3 = Task(
        ...
        repository="api-docs-test",
        priority="low",
        labels=json.dumps(["documentation", "api"]),
        ...
        required_skills=json.dumps({"documentation": 0.8, "api": 0.5}),
        status="open"
    )
    ...
    task4 = Task(
        ...
        repository="ml-service-test",
        priority="critical",
        labels=json.dumps(["machine-learning", "algorithms", "backend"]),
        technical_complexity=0.9,
        ...
        complexity_confidence=0.7,
        required_skills=json.dumps({"python": 0.8, "machine_learning": 0.9, "algorithms": 0.8}),
        status="open"
    )
...
class TestAssignmentPerformance:
    ...
    async def test_optimization_performance(self, db_session, assignment_optimizer):
        ...
        # Create proper skill vectors
        base_skill_vector = [0.5] * 5 + [0.0] * 763
        ...
        for i in range(10):
            dev = Developer(
                ...
                skill_vector=json.dumps(base_skill_vector),
                primary_languages=json.dumps({"python": 0.7}),
                domain_expertise=json.dumps({"backend": 0.6}),
                collaboration_score=0.7,
                learning_velocity=0.6
            )
        ...
        for i in range(15):
            task = Task(
                ...
                complexity_confidence=0.8,
                required_skills=json.dumps({"python": 0.6}),
                status="open"
            )
            tasks.append(task)
            db_session.add(task)
