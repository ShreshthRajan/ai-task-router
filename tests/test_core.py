...
class TestRequirementParser:
   ...
   def test_determine_scope(self, requirement_parser):
        ...
        # Create mock requirements manually for testing
        def create_mock_req(complexity):
        ...
        # Large scope
        large_reqs = [
            create_mock_req(0.8) for _ in range(8)
        ]
...
class TestTaskAPIIntegration:
   ...
   def test_complexity_analysis_structure(self, sample_complexity_request):
        ...
        # Check for security skill detection (more flexible)
        security_detected = any('security' in skill.lower() for skill in result.required_skills.keys()) or \
                        any('auth' in skill.lower() for skill in result.required_skills.keys()) or \
                        'security' in result.required_skills
        assert security_detected, f"Security skills not detected. Found skills: {list(result.required_skills.keys())}"
